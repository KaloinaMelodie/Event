...
public class LieuController {
    ...
    public ModelAndView createe(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            Part filePart = request.getPart("photoLieu");
            String base=PicHandler.toBase64(filePart);
            obj.setPhotoLieu(base);
            LieuService.create(obj, null);
    ...
    public ModelAndView updatee(@PathVariable("id") String id, HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            obj.setIdLieu(id);
            Part filePart = request.getPart(id+"modifphotoLieu");
            String base=PicHandler.toBase64(filePart);
            obj.setPhotoLieu(base);
            LieuService.update(obj, null);
    ...
    @RequestMapping("/generalLieu")
    public ModelAndView pagegenerallieu(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView();
        try {
            String sql = "select * from v_lieu where 1<2 ";
            V_lieu objfiltre = new V_lieu();
            ...
            ArrayList list = new V_lieu().select(null, sql);

            mv.addObject("list", list);
            mv.addObject("objfiltre", objfiltre);
            mv.addObject("searchform", form);

            String but = "views/general/generalLieu.jsp";
