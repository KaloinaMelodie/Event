...
public class ArtisteController {
    ...
    public ModelAndView createe(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            Artiste obj = new Artiste();
            obj.setFieldsVal(request.getParameterMap(), false, "");
            Part filePart = request.getPart("photoArtiste");
            String base=PicHandler.toBase64(filePart);
            obj.setPhotoArtiste(base);
            ArtisteService.create(obj, null);
    ...
    public ModelAndView updatee(@PathVariable("id") String id, HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            obj.setIdArtiste(id);
            Part filePart = request.getPart(id+"modifphotoArtiste");
            String base=PicHandler.toBase64(filePart);
            obj.setPhotoArtiste(base);
            ArtisteService.update(obj, null);
    ...
    public ModelAndView deletee(@PathVariable("id") String id, HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            Artiste obj = new Artiste();
            obj.setIdArtiste(id);
            ArtisteService.delete(obj, null);
    ...
    @RequestMapping("/generalArtiste")
    public ModelAndView pagegeneralartiste(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView();
        try {
            String sql = "select * from V_artiste where 1<2 ";
            V_artiste objfiltre = new V_artiste();
            ...
            ArrayList list = new V_artiste().select(null, sql);

            mv.addObject("list", list);
            mv.addObject("objfiltre", objfiltre);
            mv.addObject("searchform", form);

            String but = "views/general/generalArtiste.jsp";
