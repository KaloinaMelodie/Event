...
import com.example.event.categoriePlace.CategoriePlaceService;
...
@Controller
@RequestMapping(value = "categoriePlaceDevis")
public class CategoriePlaceDevisController {
    ...
    public ModelAndView createe(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            CategoriePlaceDevis obj = new CategoriePlaceDevis();
            obj.setFieldsVal(request.getParameterMap(), false, "");
            CategoriePlaceDevisService.create(obj, null);
    ...
    public ModelAndView updatee(@PathVariable("id") String id, HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            obj.setIdCategoriePlaceDevis(id);
            CategoriePlaceDevisService.update(obj, null);
    ...
    public ModelAndView deletee(@PathVariable("id") String id, HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            CategoriePlaceDevis obj = new CategoriePlaceDevis();
            obj.setIdCategoriePlaceDevis(id);
            CategoriePlaceDevisService.delete(obj, null);
    ...
    @RequestMapping("/generalCategoriePlaceDevis")
    public ModelAndView pagegeneralcategoriePlaceDevis(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView();
        try {
            String sql = "select * from CategoriePlaceDevis where 1<2 ";
            CategoriePlaceDevis objfiltre = new CategoriePlaceDevis();
            ...
            ArrayList list = new CategoriePlaceDevis().select(null, sql);

            mv.addObject("list", list);
            mv.addObject("objfiltre", objfiltre);
            mv.addObject("searchform", form);

            String but = "views/general/generalCategoriePlaceDevis.jsp";
    ...
    public ModelAndView insererModif(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            CategoriePlaceDevis obj = new CategoriePlaceDevis();
            obj.setFieldsVal(request.getParameterMap(), false, "");
            CategoriePlaceDevisService.insererModifier(obj);
            mv.addObject("attributesname", "");
            mv.setViewName("/main/back.do");
        } catch (Exception ex) {
            mv.addObject("error", ex.getMessage());
