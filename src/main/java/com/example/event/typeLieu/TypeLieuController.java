...
public class TypeLieuController {
    ...
    public ModelAndView createe(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TypeLieu obj = new TypeLieu();
            obj.setFieldsVal(request.getParameterMap(), false, "");
            TypeLieuService.create(obj, null);
    ...
    public ModelAndView updatee(@PathVariable("id") Integer id, HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView();
        try {
            System.out.println("update");
            TypeLieu obj = new TypeLieu();
            obj.setFieldsVal(request.getParameterMap(), true, Integer.toString(id));
            obj.setIdTypeLieu(id);
            TypeLieuService.update(obj, null);
    ...
    public ModelAndView deletee(@PathVariable("id") Integer id, HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView();
        try {
            System.out.println("delete");
            TypeLieu obj = new TypeLieu();
            obj.setIdTypeLieu(id);
            TypeLieuService.delete(obj, null);
    ...
    @RequestMapping("/generalTypeLieu")
    public ModelAndView pagegeneraltypeLieu(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView();
        try {
            String sql = "select * from TypeLieu where 1<2 ";
            TypeLieu objfiltre = new TypeLieu();
            ...
            ArrayList list = new TypeLieu().select(null, sql);

            mv.addObject("list", list);
            mv.addObject("objfiltre", objfiltre);
            mv.addObject("searchform", form);

            String but = "views/general/generalTypeLieu.jsp";
