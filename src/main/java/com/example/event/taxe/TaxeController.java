...
public class TaxeController {
    ...
    @RequestMapping("/generalTaxe")
    public ModelAndView pagegeneraltaxe(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView();
        try {
            String sql = "select * from Taxe where 1<2 ";
            Taxe objfiltre = new Taxe();
            ...
            ArrayList list = new Taxe().select(null, sql);

            mv.addObject("list", list);
            mv.addObject("objfiltre", objfiltre);
            mv.addObject("searchform", form);

            String but = "views/general/generalTaxe.jsp";
